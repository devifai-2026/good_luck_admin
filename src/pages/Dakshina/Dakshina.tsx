...
const Dakshina = () => {
  ...
  const handleAddDakshina = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Dakshina added successfully!");
      ...
      toast.error(error.response?.data?.message || "Failed to add Dakshina");
    } finally {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          toast.success("Dakshina deleted successfully!");
        } catch (error: any) {
          console.error("Error deleting Dakshina:", error);
          toast.error(error.response?.data?.message || "Failed to delete Dakshina");
        }
      }
    });
  };

  // Handle Update Dakshina
  const handleUpdateDakshina = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!godName || !day) {
      toast.error("Please enter god name and day!");
      return;
    }

    setButtonLoading(true);
    try {
      let imageURL = dakshinaList.find((r) => r._id === editingId)?.image;
      ...
      toast.success("Dakshina Updated Successfully");
  ...
  const fetchDakshinaByDay = async (selectedDay: string) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get(`/dakshina/get/${selectedDay}`);
      if (response.data?.data?.length > 0) {
        setDakshinaList(response.data.data);
      } else {
        setDakshinaList([]);
        toast.error('No Dakshina data found for the selected day.');
      }
    } catch (error: any) {
      console.error('Error fetching Dakshina by day:', error);
    ...
    <>
            ...
            <tbody>
              {loading ? (
                ...
                dakshinaList.map((dakshina) => (
                  <tr key={dakshina._id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-2 text-center">
                      <button onClick={() => handleEditClick(dakshina)} className="text-blue-500 hover:text-blue-700">
                        <FaEdit />
                      </button>
                    </td>
                    <td className="px-4 py-2 text-center">
                      <button onClick={() => handleDeleteDakshina(dakshina._id)} className="text-red-500 hover:text-red-700">
                        <FaTrash />
                      </button>
                    </td>
                  </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white dark:bg-gray-800 dark:text-gray-300 p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4">
              {isEditing ? "Update Dakshina" : "Add Dakshina"}
            </h2>
            <form onSubmit={isEditing ? handleUpdateDakshina : handleAddDakshina}>
              <div className="mb-4">
                <label className="block text-sm font-medium mb-1">God Name</label>
                <input
                  type="text"
                  placeholder="Enter God Name"
                  className="border p-2 w-full rounded dark:bg-gray-700 dark:text-gray-300"
                  value={godName}
              ...
              <div className="flex justify-between gap-3">
                ...
                <button 
                  type="submit" 
                  className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded flex-1 flex items-center justify-center"
...
// Fix the React display name error
Dakshina.displayName = "Dakshina";
